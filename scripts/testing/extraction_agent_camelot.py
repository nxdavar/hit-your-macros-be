...
def main():
    ...
    for page in nutrition_pdf_pages:
        ...
        for camelot_table in camelot_table_arr:
            ...
            prompt_template_str = """
                Please convert the following nutritional information outputted from the camelot-py library
                {nutritional_information} into a csv file. Ensure the menu item is the first column of the csv.
                """
            ...
            csv_filtered = res.content[
                res.content.find("```csv")
                + 6 : res.content.find("```", res.content.find("```csv") + 6)
            ]
            print(csv_filtered)
